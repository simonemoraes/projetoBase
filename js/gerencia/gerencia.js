$(document).ready(function () {
    ...
    $("#btn_painel_editar").click(function () {
        ...
        var ukey_gerencia = $("input[type=checkbox][name = 'check[]']:checked").attr("id");
        var url = $(this).attr("itemid");
...
function  preencherObjetoGerencia(data) {
    
    $("#ukey").val(data.ukey);
    $("#select_gerente").attr('disabled', "disabled");
    $("#select_equipe").attr('disabled', "disabled");
    $("#select_gerente option").html(data.nome_supervisor).attr('disabled', "disabled");
    ...
    if(data.data_fim === "0000-00-00"){
        $("#exampleModalLabel").html("Deligamento de Gerncia");
    }else{
        $("#exampleModalLabel").html("Gerencia já está desligada");
...
function  salvarGerencia(url) {
    ...
    var gerente = $("#select_gerente option:selected").val();
    ...
    $.post(url, {
        ...
        data_fim: data_fim
            ...
            function (data, status) {
                ...
                if (data === "sucesso") {
                    $("#msg_sucesso").html("Gerente deligado com sucesso!");
                    $("#msg_sucesso").show();
                    ...
                    if (data === "existe") {
                        $("#msg_error").html("Gerente já está associado a essa equipe na data informada e não pode ser associado novamente!");
                        $("#msg_error").show();
